...
def create_block(index, previous_hash, messages, creator, unix_timestamp=None):
    ts = int(unix_timestamp if unix_timestamp is not None else unix_now())
    block = {
        "Schema": SCHEMA,
        "Block No": int(index),
        "Timestamp": ts,
...
def create_genesis_block():
    return create_block(1, ZERO_HASH, [], "ferrofy:first-block", unix_now())
...
def build_medical_record(document, data_node_id="", balance_before=None):
    ...
    if balance_before is None:
        ...
        balance_after = int(balance_before) - cost
...
def normalize_block(block):
    ...
    normalized = create_block(block_number(block), previous_hash, block_messages(block), creator, timestamp)
    return normalized
